...
// ---- BIẾN TOÀN CỤC CHO TÍNH NĂNG HIỂN THỊ ẢNH ----
let clickCount = 0;
...
// ---- TẠO OVERLAY FULLSCREEN CHO ẢNH ----
function createFullscreenOverlay() {
  ...
  // Event listeners để đóng overlay
  const closeOverlay = () => {
  ...
  return { overlay, img, counter: imageCounter };
}

// ---- HÀM HIỂN THỊ ẢNH FULLSCREEN ----
function showFullscreenImage(imageSrc, imageIndex, totalImages) {
  if (!fullscreenOverlay) {
    fullscreenOverlay = createFullscreenOverlay();
  }
  
  fullscreenOverlay.img.src = imageSrc;
  fullscreenOverlay.counter.textContent = `${imageIndex + 1} / ${totalImages}`;
  
  fullscreenOverlay.overlay.style.display = 'flex';
  setTimeout(() => {
    fullscreenOverlay.overlay.style.opacity = '1';
    fullscreenOverlay.img.style.transform = 'scale(1)';
  }, 10);
}
...
// ---- TẠO HIỆU UNG GỢI Ý NHẤN VÀO TINH CẦU (HINT ICON) ----
let hintIcon;
let hintText;
...
// ---- HÀM XỬ LÝ CLICK MỚI VỚI TÍNH NĂNG HIỂN THỊ ẢNH ----
function onCanvasClick(event) {
  ...
  if (intersects.length > 0) {
    ...
    if (clickCount === 1) {
      // Lần click đầu tiên - khởi động intro như cũ
      requestFullScreen();
      introStarted = true;
      fadeInProgress = true;
      ...
      // Từ lần click thứ 2 trở đi - hiển thị ảnh fullscreen
      if (heartImages && heartImages.length > 0) {
        const imageToShow = heartImages[currentImageIndex];
        showFullscreenImage(imageToShow, currentImageIndex, heartImages.length);
        
        // Chuyển sang ảnh tiếp theo, quay lại ảnh đầu nếu hết
